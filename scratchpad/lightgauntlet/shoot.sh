...
../backend/.venv/Scripts/python.exe -m roomkit.lightshot --room "$ROOM" --level "$LEVEL" --out "$OUT" "$@" > "$J" 2>&1
...
try:
    ...
    print('PARSE FAIL', e); print(raw[:3000]); raise SystemExit
print('delta', d['delta'], '| on', d['on']['meter'], '| off', d['off']['meter'])
for f in d['on']['fixtures']:
    print('  fx', f['objectId'], 'on', f['on'], 'glow', f['glow'], 'emits', f['emits'], 'shown', f['shown'], 'pooled', f['pooled'])
