...
const uniforms=()=>{clouds.children.forEach(m=>m.onBeforeRender());return clouds.children.map(m=>({name:m.name,day:m.material.uniforms.cloudDay.value,wet:m.material.uniforms.cloudWet.value,color:m.material.uniforms.cloudColor.value.toArray(),sky:m.material.uniforms.skyColor.value.toArray()}));};
const dry=uniforms();env.setGroundWet(1);const wet=uniforms();env.setGroundWet(0);
window.__daylight.simulate({elevation:-18,azimuth:335,condition:'sunny'});daylight.settleDaylight();const night=uniforms();const nightPixels=compare([18,5.7,-86],[13.5,16,-24.5]);
window.__daylight.simulate({elevation:42,azimuth:335,condition:'cloudy'});daylight.settleDaylight();const cloudy=uniforms(),cloudyPixels=compare([18,5.7,-86],[13.5,16,-24.5]);
window.__daylight.simulate({elevation:42,azimuth:335,condition:'rainy'});daylight.settleDaylight();const rainy=uniforms(),rainyPixels=compare([18,5.7,-86],[13.5,16,-24.5]);
window.__daylight.simulate({elevation:42,azimuth:335,condition:'sunny'});daylight.settleDaylight();env.setGroundWet(1);const wetPixels=compare([18,5.7,-86],[13.5,16,-24.5]);
Object.defineProperty(performance,'now',{value:originalNow,configurable:true});
return{comparisons,bounds,dry,wet,night,nightPixels,cloudy,cloudyPixels,rainy,rainyPixels,wetPixels,renderer:{exposure:renderer.toneMappingExposure,toneMapping:renderer.toneMapping,colorSpace:renderer.outputColorSpace}};
});fs.writeFileSync(__dirname+'/sky-r4-verify.json',JSON.stringify({report,errors},null,2));console.log(JSON.stringify({report,errors}));
