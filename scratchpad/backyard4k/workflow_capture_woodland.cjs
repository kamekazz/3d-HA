...
(async()=>{
  ...
  try {
    for (const name of selected) {
      ...
      progress('Capturing '+round, name+' · native 3840 × 2160 · rear daylight');
      ...
      const state = await page.evaluate(async (capturePose)=>{
        ...
        return {position:camera.position.toArray(),target:controls.target.toArray(),
          ...
          frontGeometry:{selection:'environment triangles with every world-space vertex z >= 0',triangles:frontCount,xor:frontXor,sum:frontSum},frontItems,rearTrees:env.getYardItems().filter(i=>i.label==='Rear mature broadleaf').map(i=>({key:i.key,pivot:i.pivot}))};
      },pose);
      ...
      if(Math.max(...state.canvas)>=3840){
        ...
        fs.writeFileSync(fp,JSON.stringify({...prev,renders,renderSizes,round,updated:new Date().toISOString()},null,2));
      }
    }
