  ...
  for (const name of groundNames) {
    ...
    material.onBeforeCompile = function(shader, activeRenderer) {
      ...
      if (name === 'rear-mown-turf') {
        // The snow-detail luminance was computed above this line. Correct only
        // the uncovered grass pigment, leaving full snow color/detail intact.
    ...
    mesh.onBeforeRender = function(...args) {
      ...
      if (name === 'rear-short-grass-tufts' || name === 'rear-mown-clover') {
        // Their existing hooks already interpolate snow. Tint only the dry/wet
        // grass contribution, preserving the snow-white part at every blend.
