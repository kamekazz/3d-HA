...
const root=path.resolve(__dirname,'../..'),out=path.join(__dirname,'renders/ground-r6-diagnostic');fs.mkdirSync(out,{recursive:true});
...
const data=[];
for(const trial of [{name:'baseline',pose:poses.photo12},{name:'sky-visible',pose:poses.photo12,ao:false},{name:'lower-eye',pose:lower,ao:false}]){
