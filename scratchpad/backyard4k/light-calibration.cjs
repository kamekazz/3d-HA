...
for(const light of [{elevation:42,azimuth:225,condition:'sunny'},{elevation:42,azimuth:260,condition:'sunny'},{elevation:42,azimuth:290,condition:'sunny'},{elevation:35,azimuth:225,condition:'partlycloudy'}]){
...
await p.waitForTimeout(2200);await p.evaluate(`(${setup})(${JSON.stringify(options)})`);await p.waitForTimeout(500);
