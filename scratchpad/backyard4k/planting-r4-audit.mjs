...
const before=fs.readFileSync(path.join(dir,'planting-r4-before.js'),'utf8');
...
const expected=['shrub:190:-454','shrub:220:-452','shrub:250:-449','shrub:280:-435'];
if(JSON.stringify(changed)!==JSON.stringify(expected))throw Error('Unexpected geometry changes '+JSON.stringify(changed));
const result={protocol:'Offline execution using exact Three.js r160; bed factories recorded as calls, no browser/capture',itemCount:current.items.length,identitiesUnchanged:true,bedCallsUnchanged:true,changed,unchangedGeometryCount:current.items.length-changed.length,items:current.items};
fs.writeFileSync(path.join(dir,'planting-r4-audit.json'),JSON.stringify(result,null,2));
console.log(JSON.stringify({...result,items:current.items.filter(i=>changed.includes(i.key))},null,2));
