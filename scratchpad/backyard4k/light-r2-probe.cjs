...
(async()=>{
 ...
 await p.evaluate(()=>window.__daylight.simulate({elevation:42,azimuth:335,condition:'sunny'}));await p.waitForTimeout(2000);
 ...
 const casters=[];env.traverse(o=>{if(o.isMesh&&o.castShadow&&o.visible)casters.push(o);});
 const points=[];
 for(const z of [-95,-85,-75,-65,-55,-45])for(const x of [-10,0,10,20,30,40,50]){
 ...
 fs.writeFileSync(__dirname+'/light-r2-probe.json',JSON.stringify({state,errors},null,2));console.log(JSON.stringify({state,errors}));
