...
function progress(status, details) {
  const fp = path.join(here,'progress.json');
  let prev = {}; try {prev=JSON.parse(fs.readFileSync(fp,'utf8').replace(/^\uFEFF/,''))}catch{}
  fs.writeFileSync(fp, JSON.stringify({...prev,status,details,updated:new Date().toISOString()},null,2));
}
(async()=>{
  ...
  try {
    for (const name of selected) {
      const pose = poses[name]; if(!pose) throw Error('Unknown pose: '+name);
      ...
      const page = await browser.newPage({viewport:{width:pose.size[0],height:pose.size[1]},deviceScaleFactor:1});
      ...
      page.on('crash',()=>errors.push('Browser renderer process crashed'));
      await page.goto(base,{waitUntil:'load',timeout:90000});
      console.log(JSON.stringify({name,phase:'page loaded'}));
      await page.waitForFunction(()=>!!window.__scene3d,{},{timeout:60000}).catch(error=>{
        fs.writeFileSync(path.join(outDir,name+'-startup-error.json'),JSON.stringify({round,name,errors,consoleErrors,requestsFailed,error:String(error)},null,2));
        throw error;
      });
      console.log(JSON.stringify({name,phase:'scene initialized'}));
      await page.waitForTimeout(3500);
      const options={pose,level:'all',light,markers:false,cutaway:false,exteriorsOn:false};
      await page.evaluate(`(${setup})(${JSON.stringify(options)})`);
      console.log(JSON.stringify({name,phase:'camera configured'}));
      let loaded;
      ...
      const report={round,name,url:base,pose,light,level:'all',cutaway:false,markers:false,
