...
def _pm_deck(cv):
    ...
    cv.vgrad(0.0, 0.0, 1.0, 0.056, "#c4c4c6", "#6a6a6c")        # back trim
    cv.seg((0.0, 0.064), (1.0, 0.064), "#26262e", 0.022, 0.85)

    # the pale instruction label -- the one printed thing on this deck
    cv.rect(0.706, 0.140, 0.928, 0.360, "#d0d2d6")
    cv.rect(0.718, 0.156, 0.916, 0.344, "#a6a8ae")
    ...
    for (u, v) in _PM_BTN_UV:
        _collar(cv, u, v, 0.055, "#b0b0b4", k=1.32, a=0.30, edge="#0f0f13",
                ea=0.7)
...
DECKS = {
    ...
    "pac-man": {
        ...
        "sticks": [_mk_stick(s, 0.038, 0.180, 0.120) for s in _PM_STICKS],
        "buttons": [_mk_btn(b, 0.028, 0.011, 0.74, 1.50) for b in _PM_BUTTONS],
    },
    # --------------------------------------------------------------- NBA JAM
    ...
    "nba-jam": {
        ...
        "sticks": [_mk_stick(_NBA_STICKS[0], 0.042, 0.190, 0.135),
                   _mk_stick(_NBA_STICKS[1], 0.040, 0.215, 0.135)],
        "buttons": [_mk_btn(b, 0.036, 0.015, 0.76, 1.46) for b in _NBA_BUTTONS],
    },
    # -------------------------------------------------------- TURTLES IN TIME
    ...
    "tmnt-turtles-in-time": {
        ...
        "sticks": [_mk_stick(s, 0.044, 0.200, 0.140) for s in _TM_STICKS],
        "buttons": ([_mk_btn(b, 0.046, 0.019, 0.78, 1.40) for b in _TM_BUTTONS]
                    + [_mk_btn(b, 0.018, 0.007, 0.72, 1.55)
                       for b in _TM_ADMIN]),
    },
    # --------------------------------------------------- GOLDEN TEE 3D GOLF
    ...
    "golden-tee-3d-golf": {
        ...
        "trackball": {"u": _GT_TRACK[0], "v": _GT_TRACK[1], "r": _GT_TRACK[2],
                      "color": _GT_TRACK[3], "bezel_r": 0.163,
                      "bezel_color": "#161c12"},
        "buttons": [_mk_btn(b, 0.030, 0.012, 0.74, 1.52) for b in _GT_BUTTONS],
    },
}
