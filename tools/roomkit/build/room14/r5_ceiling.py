"""Vaulted ceiling — round 5 = round 4 plus the recessed-can winding fix.

BUG FIXED THIS ROUND: from the dollhouse poses the six recessed cans
rendered as white blobs lying on the FLOOR.  `roomkit.glb.cylinder` builds
BOTH end caps with the same winding, so its bottom cap faces UP and its top
cap faces DOWN.  With `double_sided=False` (which the ceiling needs, so the
slopes cull from above and you can see into the room) that left one bright
emissive disc per can facing straight up through the culled ceiling -- and
at a 50-degree dollhouse elevation a disc 14 ft up projects onto the floor.
The cans are now a single DOWN-facing disc each, wound explicitly.
...
def disc_down(r, seg):
    """A flat disc whose ONE face points -Y.  See the module docstring: the
    generic cylinder() winds both caps the same way, so its up-facing cap
    survives the ceiling's back-face cull and shows from the dollhouse pose."""
    v = [(0.0, 0.0, 0.0)] + [(r * math.cos(2 * math.pi * i / seg), 0.0,
...
for (cx, cz) in CANS:
    ...
    rim, ap = made[b]
    y = h(cx)
    m.add(disc_down(0.27, 20), rim, at=(cx, y - 0.012, cz), rot_z=rot)
    m.add(disc_down(0.21, 20), ap, at=(cx, y - 0.030, cz), rot_z=rot)
